...
#[cfg(target_arch = "wasm32")]
fn dispatch_custom_event_inner(event: Result<CustomEvent, JsValue>, event_target: &EventTarget) -> Result<bool, EventDispatchError> {
    match event {
        Ok(event) => {
            match event_target.dispatch_event(&event) {
                Ok(result) => Ok(result),
                Err(error) => {
                    log::error!("Could not dispatch event: {:?}", error);
...
#[cfg(target_arch = "wasm32")]
pub fn dispatch_event_target_event_with_data<T: Serialize>(event_type: &str, data: &T, event_target: &EventTarget) -> Result<bool, EventDispatchError> {
...
#[cfg(target_arch = "wasm32")]
pub fn dispatch_event_target_event(event_type: &str, event_target: &EventTarget) -> Result<bool, EventDispatchError> {
    dispatch_custom_event_inner(
...
#[cfg(target_arch = "wasm32")]
pub fn dispatch_canvas_event_with_data<T: Serialize>(event_type: &str, data: &T, canvas: &HtmlCanvasElement) -> Result<bool, CanvasEventDispatchError> {
...
#[cfg(target_arch = "wasm32")]
pub fn dispatch_canvas_event(event_type: &str, canvas: &HtmlCanvasElement) -> Result<bool, CanvasEventDispatchError> {
    let canvas_clone = canvas.clone();
    match canvas_clone.dyn_into::<EventTarget>() {
        Ok(event_target) => {
            match dispatch_event_target_event(event_type, &event_target) {
                Ok(result) => Ok(result),
                Err(error) => Err(CanvasEventDispatchError::EventDispatch(error))
            }
        },
